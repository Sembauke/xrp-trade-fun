    ...
    <div className="min-h-screen bg-surface-950 flex flex-col">
      ...
      {isLoading ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-slate-400 text-sm animate-pulse">Verbinden met marktdata…</p>
        </div>
      ) : (
        <main className="flex-1 p-4 lg:p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="card flex flex-col justify-between gap-3">
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  className="text-[11px] px-2.5 py-1 rounded-md bg-surface-700 text-slate-300 hover:bg-surface-700/80"
                  onClick={() => setShowAdvanced((prev) => !prev)}
                >
                  {showAdvanced ? 'Minder details' : 'Meer details'}
                </button>
              </div>
              {indicators && (
          ...
          {/* Signals */}
          {showAdvanced && decision && decision.signals.length > 0 && (
